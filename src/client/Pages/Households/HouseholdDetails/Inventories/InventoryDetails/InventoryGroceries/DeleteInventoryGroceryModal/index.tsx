...
import { useDeleteGroceryFromInventoryMutation, useGetInventoriesQuery } from 'client/Redux/api/inventories'
...
const DeleteInventoryGroceryModal = ({ householdId, inventoryGrocery, modal }: Props) => {
  ...
  const { data: inventories} =
    useGetInventoriesQuery(householdId, {
      refetchOnMountOrArgChange: true,
    })
  
  const [removeGroceryFromInventory] = useDeleteGroceryFromInventoryMutation()

  const handleDeleteInventoryGrocery = useCallback(
    async () => {
    ...
    [removeGroceryFromInventory, inventoryGrocery.grocery_id, inventoryGrocery.inventory_id, householdId, modal]
  )
  return (
    <modal.RenderModal titleColor="white" titleLabel="Delete inventory grocery">
      <>
        <ModalBodyWrapper hasFooter>
          Do you really want to delete grocery "{groceries?.find(
                        (g: Grocery) => g.id === inventoryGrocery.grocery_id
                      )?.grocery_name}" from inventory "
          {inventories?.find(
